...
## 基本原則
* 変わるものを変わらないものから分離
* インターフェイスに対してプログラミングし、実装に対して行わない(他クラスを直接呼ばず、DIしたクラスのパブリックインターフェースのみに依存する)
* 継承より集約
* 委譲、委譲、委譲(delegation)
...
| [ビルダー](https://github.com/rikoroku/ruby_design_patterns/tree/master/builder) | 複雑なオブジェクトを生成する時に使用するもの |
